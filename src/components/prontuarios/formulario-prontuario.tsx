...
function CampoPacienteTravado({ paciente, erro }: { paciente: PacienteDoProntuario; erro?: string }) {
  return <Campo id="paciente_id" rotulo="Paciente" obrigatorio erro={erro}>
    ...
    <div className="rounded-[var(--radius-cartao)] border border-outline-variant bg-surface px-3.5 py-2.5"><p className="text-sm font-medium text-on-surface">{paciente.nome}</p><p className="mt-0.5 text-xs text-outline">{paciente.detalhe}</p></div>
  </Campo>;
}

function CampoAtendimento({ atendimento }: { atendimento: AtendimentoDoProntuario | null }) {
  return <><input type="hidden" name="atendimento_id" value={atendimento?.id ?? ""} />{atendimento ? <div className="rounded-[var(--radius-cartao)] border border-card-border bg-surface-container-low px-3.5 py-2.5"><p className="text-xs font-medium uppercase text-outline">Atendimento vinculado</p><p className="mt-1 text-sm text-on-surface">{atendimento.detalhe}</p></div> : null}</>;
}
...
export function FormularioProntuario({ acao, modo, inicial, prontuarioId, pacienteInicial, atendimentoInicial, dataPadrao, cancelarPara, rotuloSalvar }: { acao: Acao; modo: "novo" | "versao"; inicial?: Partial<ValoresProntuario>; prontuarioId?: string; pacienteInicial: PacienteDoProntuario | null; atendimentoInicial?: AtendimentoDoProntuario | null; dataPadrao: string; cancelarPara: string; rotuloSalvar: string }) {
  ...
  return <form action={enviar} className="flex flex-col gap-8" noValidate>
    {prontuarioId ? <input type="hidden" name="prontuario_id" value={prontuarioId} /> : null}
    {erros.geral || erros.prontuario_id ? <p role="alert" className="flex items-start gap-2 rounded-[var(--radius-cartao)] border border-error/25 bg-error-container px-3.5 py-2.5 text-sm text-on-error-container"><CircleAlert aria-hidden="true" size={16} className="mt-0.5 shrink-0" />{erros.geral ?? erros.prontuario_id}</p> : null}

    <GrupoDeCampos titulo="Identificação"><div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
    ...
    {modo === "versao" ? <GrupoDeCampos titulo="Motivo da versão"><Campo id="motivo" rotulo="Motivo" obrigatorio erro={erros.motivo}><input id="motivo" name="motivo" type="text" required maxLength={LIMITES_PRONTUARIO.motivo} defaultValue={de("motivo")} placeholder="Evolução após retorno" className={cn(ENTRADA, erros.motivo && ENTRADA_ERRO)} {...marcar("motivo")} /></Campo></GrupoDeCampos> : null}

    <GrupoDeCampos titulo="Registro clínico" descricao="Pelo menos um campo desta ficha precisa ser preenchido."><div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
