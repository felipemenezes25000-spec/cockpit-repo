...
export function CommandPalette({
  ...
  const itens = useMemo(() => {
    ...
    const modulos = MENU.filter((item) => {
  ...
  const total = itens.length + (podeBuscar ? 1 : 0);
  ...
  function aoTeclar(evento: ReactKeyboardEvent<HTMLDivElement>) {
    ...
    if (evento.key === "Enter" && total > 0) {
      ...
      const indiceModulo = selecionado - (podeBuscar ? 1 : 0);
      const item = itens[indiceModulo];
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative border-b border-card-border/80 px-4 py-4 sm:px-5">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <input
              ...
              placeholder="Ir para um módulo ou buscar no sistema…"
        ...
        <div className="rolagem-discreta max-h-[58vh] overflow-y-auto p-2.5 sm:p-3">
          {podeBuscar ? (
            <button
              type="button"
          ...
          <div className="px-2 pb-1 pt-2 text-[0.66rem] font-semibold tracking-[0.08em] text-outline uppercase">
            Módulos
          </div>
          ...
          {itens.length ? (
            <div className="flex flex-col gap-1">
              {itens.map((item, indice) => {
                ...
                const indiceReal = indice + (podeBuscar ? 1 : 0);
                ...
                return (
                  ...
                  >
                    <span className={cn(
                      ...
                      ativo
                        ? "border-primary/10 bg-primary-fixed/60 text-primary"
                        : "border-card-border/80 bg-white/65 text-on-surface-variant",
                    )}>
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center gap-2 text-sm font-semibold text-on-surface">
                        {item.rotulo}
                        {item.emConstrucao ? (
                          <span className="rounded-[6px] border border-card-border bg-white/70 px-1.5 py-0.5 text-[0.58rem] font-semibold tracking-wide text-outline uppercase">em breve</span>
                        ) : null}
                      </span>
                    ...
                    <ArrowRight
                      aria-hidden="true"
                      size={16}
                      className={cn("shrink-0 text-outline transition-transform duration-150", ativo && "translate-x-0.5 text-primary")}
                    />
                  </button>
                );
              })}
            ...
            <div className="px-4 py-8 text-center">
              <p className="text-sm font-medium text-on-surface">Nenhum módulo encontrado</p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 border-t border-card-border/75 bg-white/40 px-4 py-2.5 text-[0.64rem] text-outline sm:px-5">
          ...
          <span className="inline-flex items-center gap-1.5">
            <kbd className="rounded-[6px] border border-card-border bg-white/80 px-1.5 py-0.5 font-semibold">Ctrl K</kbd>
            comandos rápidos
          </span>
        </div>
      </div>
