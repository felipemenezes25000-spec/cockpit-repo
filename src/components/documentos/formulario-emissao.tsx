...
import { Campo, ENTRADA, GrupoDeCampos } from "@/components/ui/field";
...
export function FormularioEmissao({
  ...
  return (
    <form action={enviar} className="flex flex-col gap-6 sm:gap-7">
      {corrigeId ? <input type="hidden" name="documento_anterior_id" value={corrigeId} /> : null}

      {corrigeId ? (
        <p className="flex items-start gap-3 rounded-[var(--radius-painel)] border border-informativo-borda bg-informativo-fundo px-4 py-3 text-sm leading-6 text-on-surface-variant shadow-[0_12px_30px_-26px_rgba(8,84,160,.35)]">
          <span className="flex size-8 shrink-0 items-center justify-center rounded-[var(--radius-controle)] border border-informativo-borda bg-surface/70 text-informativo-texto"><Replace aria-hidden="true" size={16} /></span>
          <span>Este documento vai corrigir um anterior. Ao emitir, o antigo passa a &quot;substituído&quot; — a menos que já esteja assinado, e aí ele permanece como está.</span>
        </p>
      ) : null}
