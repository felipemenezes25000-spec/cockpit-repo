  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <select name="situacao" value={situacao} onChange={(evento) => navegar({ situacao: evento.target.value })} aria-label="Filtrar por situação" className={seletor}>
          <option value="">Todas as situações</option>
          {SITUACOES.map((valor) => <option key={valor} value={valor}>{ROTULO_SITUACAO[valor]}</option>)}
        </select>

        <select name="tipo" value={tipo} onChange={(evento) => navegar({ tipo: evento.target.value })} aria-label="Filtrar por tipo" className={seletor}>
