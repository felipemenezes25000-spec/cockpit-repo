...
export function AvisoDeTela({
  ...
  return (
    <section className={cn("premium-panel relative mx-auto w-full max-w-2xl overflow-hidden rounded-[var(--radius-painel)] border px-6 py-10 text-center sm:px-10 sm:py-12", className)}>
      ...
      <p className={cn("rotulo relative mt-6", tom === "negativo" ? "text-negativo" : "text-primary")}>{rotulo}</p>
      <h1 ref={tituloRef} tabIndex={tituloRef ? -1 : undefined} className="titulo-tela relative mx-auto mt-2 max-w-xl text-on-surface outline-none">
        {titulo}
      </h1>
      <div className="relative mx-auto mt-3 max-w-lg text-sm leading-6 text-on-surface-variant">{children}</div>
      {selos ? <div className="relative mt-6 flex flex-wrap justify-center gap-2">{selos}</div> : null}
      {acoes ? <div className="relative mt-7 flex flex-wrap justify-center gap-2">{acoes}</div> : null}
    </section>
  );
}
