...
export function CardRecolhivel({
  ...
  return (
    ...
    >
      <div className={cn("recolhivel-topo flex flex-wrap items-center justify-between gap-4 bg-[linear-gradient(180deg,#ffffff_0%,#fcfdff_100%)] px-4 py-4.5 sm:px-6", aberto && "recolhivel-topo-aberto")}>
        <div className="min-w-0 flex-1">
          <h2 className="titulo-secao text-on-surface">
            <button
              ...
              className="recolhivel-gatilho group -ml-1.5 inline-flex max-w-full items-center gap-2 rounded-[var(--radius-controle)] px-1.5 py-0.5 text-left transition-[background-color,color,transform] hover:bg-selecao hover:text-primary active:scale-[0.99]"
            >
              <span className="min-w-0">{titulo}</span>
              <span aria-hidden="true" className="recolhivel-seta-caixa flex size-6 shrink-0 items-center justify-center rounded-full border border-card-border bg-surface text-outline shadow-[0_7px_16px_-14px_rgba(8,41,76,.45)] transition-colors group-hover:border-primary-fixed-dim group-hover:text-primary">
                <ChevronDown size={15} strokeWidth={2} className="recolhivel-seta" />
              </span>
            </button>
          </h2>
          {descricao ? <div className="mt-1 max-w-3xl text-sm leading-6 text-on-surface-variant">{descricao}</div> : null}
        </div>
