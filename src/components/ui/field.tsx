...
export function Campo({ id, rotulo, dica, erro, obrigatorio = false, children, className }: { id: string; rotulo: string; dica?: string; erro?: string; obrigatorio?: boolean; children: ReactNode; className?: string }) {
  return (
    <div className={cn("group/campo flex min-w-0 flex-col gap-1.5", className)}>
      <label
        htmlFor={id}
        className={cn(
      ...
      {erro ? (
        <p id={`${id}-erro`} role="alert" className="mt-0.5 flex min-w-0 items-start gap-1.5 break-words text-xs font-medium leading-5 text-error">
          <span aria-hidden="true" className="mt-[0.42rem] size-1.5 shrink-0 rounded-full bg-error" />
          <span className="min-w-0">{erro}</span>
        </p>
      ) : dica ? (
        <p id={`${id}-dica`} className="mt-0.5 min-w-0 break-words text-xs leading-5 text-outline transition-colors group-focus-within/campo:text-on-surface-variant">
          {dica}
        </p>
      ) : null}
    </div>
...
export function GrupoDeCampos({ titulo, descricao, children, className }: { titulo: string; descricao?: string; children: ReactNode; className?: string }) {
  return (
    <fieldset
      className={cn(
        "premium-panel min-w-0 rounded-[var(--radius-painel)] border border-card-border bg-surface px-4 py-5 shadow-[0_14px_34px_-30px_rgba(8,41,76,.38)] sm:px-6 sm:py-6",
        className,
      )}
    >
      <legend className="sr-only">{titulo}</legend>
      <div className="mb-5 min-w-0 border-b border-card-border pb-4">
        <p className="titulo-secao break-words text-primary">{titulo}</p>
        {descricao ? <p className="mt-1.5 max-w-2xl break-words text-xs leading-5 text-outline">{descricao}</p> : null}
      </div>
      {children}
    </fieldset>
  );
