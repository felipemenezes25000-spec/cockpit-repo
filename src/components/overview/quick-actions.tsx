      ...
      <div className="grid grid-cols-2 gap-2.5 md:flex md:flex-wrap">
        {acoes.map(({ rotulo, href, icone: Icone }, i) => (
          ...
          >
            ...
            <span className={cn(
              "relative flex size-8 shrink-0 items-center justify-center rounded-[10px] transition-[transform,background-color] duration-200 group-hover:scale-[1.06]",
              i === 0 ? "bg-white/13" : "bg-primary-fixed/50 group-hover:bg-primary-fixed/72",
            )}>
              <Icone aria-hidden="true" size={17} strokeWidth={1.65} />
            </span>
            <span className="relative min-w-0 flex-1 text-left leading-tight">{rotulo}</span>
            <ArrowUpRight aria-hidden="true" size={15} strokeWidth={1.7} className="relative hidden shrink-0 opacity-55 transition-[transform,opacity] duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:opacity-90 sm:block" />
          </Link>
        ))}
      </div>
    </section>
