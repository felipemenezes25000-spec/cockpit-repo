...
import type { CatalogoAgenda, OpcaoProcedimento } from "@/server/consultas/agenda";
import type { EstadoAtendimento, PacienteParaSelecao } from "@/server/acoes/agenda";
...
export function FormularioAtendimento({
  ...
  const de = (campo: keyof ValoresAtendimento) => estado.valores?.[campo] ?? inicial[campo];
  ...
  function aoTrocarProcedimento(id: string) {
    const procedimento: OpcaoProcedimento | undefined = catalogo.procedimentos.find((p) => p.id === id);
    if (!procedimento) return;
    setDuracao(String(procedimento.duracaoMin));
    setValor(procedimento.valorPadrao > 0 ? procedimento.valorPadrao.toFixed(2).replace(".", ",") : "");
  }
  ...
  useEffect(() => {
    ...
    const alvo = formulario.current?.querySelector<HTMLElement>('[aria-invalid="true"]') ?? formulario.current?.querySelector<HTMLElement>("[data-erro-geral]");
    alvo?.focus();
  }, [estado]);
